...
import { Calendar, Clock, Plus, Trash2, Edit3, ToggleLeft, ToggleRight, Repeat, Target, Video, Minus } from 'lucide-react'
import ExecutionTimeline from '../components/ExecutionTimeline'

interface ShortsPlanningConfig {
  channel_id: number; name: string; slug: string
  shorts_per_day: number; shorts_enabled: boolean
}
    ...
    <div className="bg-dark-700/50 rounded-xl p-4 space-y-3 border border-surface-border">
      ...
      {config.shorts_enabled && (
        <div className="flex items-center justify-between text-xs">
          <span className="text-gray-400">Shorts/día</span>
          <div className="flex items-center gap-2">
            <button onClick={() => onUpdate({ shorts_per_day: Math.max(1, config.shorts_per_day - 1) })}
            ...
            <span className="text-white font-mono w-4 text-center">{config.shorts_per_day}</span>
            <button onClick={() => onUpdate({ shorts_per_day: Math.min(8, config.shorts_per_day + 1) })}
...
function ShortsSection() {
  ...
  return (
    <section className="glass rounded-xl p-5">
      <h3 className="font-display text-base font-semibold text-white mb-4 flex items-center gap-2"><Video size={16} className="text-purple-400" /> Planificación de Shorts</h3>
