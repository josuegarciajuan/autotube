...
# ago 2026 (antiban): 3h → 6h → 24h — las ráfagas de publicación del mismo canal
# alimentaron el flag de spam de YouTube (strikes de ago-2026). 24h = máx 1
# publicación/día, coherente con SAME_CHANNEL_PUBLISH_GAP_HOURS y
# MAX_LONGFORM_PUBLISH_PER_DAY (pipeline/publish_scheduler.py).
MIN_SAME_CHANNEL_PUBLISH_GAP_HOURS = 24  # minimum hours between publish times for same channel (v10.1 collision fix; antiban 24h)
...
def _generate_and_publish_native_short(channel_id: int, channel_slug: str, db=None):
    ...
    if yt_id:
        ...
        cursor = conn.execute("INSERT INTO shorts (channel_id, type, title, hook_title, hook_text, topic, status, file_path, youtube_id, youtube_url, published_at, publish_at, yt_visibility, actual_published_at, has_subscribe_cta, longform_linked, longform_linked_at) VALUES (?, 'native', ?, ?, ?, ?, 'published', ?, ?, ?, datetime('now'), NULL, 'public', datetime('now'), ?, 1, datetime('now','localtime'))", (channel_id, title, title[:60], hook_text, topic, str(video_path), yt_id, result.get("url", ""), int(has_subscribe_cta)))
