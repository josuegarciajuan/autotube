...
def _generate_and_publish_native_short(channel_id: int, channel_slug: str):
    ...
    response = client.chat.completions.create(
        model=LLM_MODEL,
        messages=[{"role": "user", "content": f"Genera un Short viral en español de ~40-45 segundos (~70-90 palabras). Canal: {display_name} — {niche}. Tagline: {tagline}. Usa 5 bloques (hook, desarrollo1, desarrollo2, climax, cierre), cada uno con 1-2 frases concisas. Devuelve SOLO JSON: {{\"titulo\": \"...\", \"hook_text\": \"frase de gancho 8-12 palabras\", \"bloques\": [{{\"tipo\": \"hook\", \"texto\": \"1-2 frases\"}}, {{\"tipo\": \"desarrollo1\", \"texto\": \"1-2 frases contexto\"}}, {{\"tipo\": \"desarrollo2\", \"texto\": \"1-2 frases dato impactante\"}}, {{\"tipo\": \"climax\", \"texto\": \"1-2 frases consecuencia\"}}, {{\"tipo\": \"cierre\", \"texto\": \"1-2 frases cierre + suscríbete\"}}]}}. NADA MÁS."}],
