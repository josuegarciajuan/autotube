...
def generate_playlists_for_channel(
    ...
    system_prompt = (
        ...
        f"**CRÍTICO: La keyword principal del nicho debe ser la PRIMERA palabra del nombre.** "
        f"Ejemplo correcto: 'Misterios Históricos Sin Resolver' (empieza con keyword). "
        f"Ejemplo INCORRECTO: 'Lo Más Increíble del Canal' (genérico, sin keyword). "
        f"NUNCA uses nombres genéricos como 'Lo Mejor De', 'Mis Favoritos', 'Increíble'.\n"
        f"2. Una descripción SEO {lang_instruction} (150-200 caracteres máx) que incluya "
        f"la keyword principal en la primera frase y un CTA suave al final.\n"
