...
def migrate_v2(db_path: str = None):
    ...
    # ── v45: social account identity fields (email, login password, notes) ──
...
def _migrate_v45(conn, logger):
    """Idempotent v45: social account identity fields.
    ...
    for col, col_type in [
        ...
        if col not in cols:
            try:
                ...
                logger.info("Migration v45: added channel_social_accounts.%s", col)
            except Exception as exc:
                logger.warning("Migration v45: failed to add channel_social_accounts.%s: %s", col, exc)
    conn.commit()
    logger.info("Migration v45: social account identity fields ensured")
